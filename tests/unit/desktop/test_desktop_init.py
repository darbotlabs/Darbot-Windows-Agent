...
from darbot_windows_agent.desktop.views import DesktopState, App, Size
from darbot_windows_agent.desktop.config import EXCLUDED_APPS, AVOIDED_APPS
# No need to import Tree here, as we will patch its source
from darbot_windows_agent.tree.views import TreeState
import uiautomation as uia
import pyautogui

class TestDesktop:
    """
    Tests for the Desktop class in darbot_windows_agent.desktop.__init__.py.
    """
    ...
    def setup_mocks(self):
        ...
        """
        # --- FIX: Patch 'Tree' at its source module 'darbot_windows_agent.tree' ---
        with patch("darbot_windows_agent.desktop.pyautogui") as mock_pyautogui, \
             patch("darbot_windows_agent.desktop.GetScreenSize") as mock_get_screen_size, \
             patch("darbot_windows_agent.desktop.GetRootControl") as mock_get_root_control, \
             patch("darbot_windows_agent.desktop.ControlFromCursor") as mock_control_from_cursor, \
             patch("darbot_windows_agent.desktop.SetWindowTopmost") as mock_set_window_topmost, \
             patch("darbot_windows_agent.desktop.subprocess.run") as mock_subprocess_run, \
             patch("darbot_windows_agent.desktop.csv") as mock_csv, \
             patch("darbot_windows_agent.desktop.io") as mock_io, \
             patch("darbot_windows_agent.desktop.process") as mock_process, \
             patch("darbot_windows_agent.desktop.sleep") as mock_sleep, \
             patch("darbot_windows_agent.tree.Tree") as MockTree:

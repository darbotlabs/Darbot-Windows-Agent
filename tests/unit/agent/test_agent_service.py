...
from darbot_windows_agent.agent.service import Agent, logger
...
from darbot_windows_agent.agent.registry.service import Registry
...
def agent_instance(
    ...
    with patch("darbot_windows_agent.agent.service.Desktop", return_value=mock_desktop), \
         patch("darbot_windows_agent.agent.service.Registry", return_value=mock_registry), \
         patch("darbot_windows_agent.agent.service.AgentState", return_value=mock_agent_state), \
         patch("darbot_windows_agent.agent.service.AgentStep", return_value=mock_agent_step):
...
class TestAgent:
    """Tests for the Agent class in darbot_windows_agent.agent.service."""

    def test_init_default_values(self):
        """Test that the Agent initializes with default values correctly."""
        with patch('darbot_windows_agent.agent.service.Registry') as mock_reg:
            agent = Agent(llm=MagicMock(spec=BaseChatModel))
            assert agent.name == "Darbot Windows Agent"
    ...
    def test_init_custom_values(self):
        ...
        with patch('darbot_windows_agent.agent.service.Registry') as mock_reg_class:
    ...
    @patch("darbot_windows_agent.agent.service.extract_agent_data")
    @patch("darbot_windows_agent.agent.service.logger")
    ...
    @patch("darbot_windows_agent.agent.service.Prompt")
    @patch("darbot_windows_agent.agent.service.image_message")
    ...
    @patch("darbot_windows_agent.agent.service.Console")
